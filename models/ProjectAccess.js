...
const ProjectAccess = {
     ...
     */
    async create({ projectId, userId, permission = 'view', status = 'pending' }) {
        const [result] = await db.query(
            `INSERT INTO project_access (project_id, user_id, permission, status)
             VALUES (?, ?, ?, ?)
             ON DUPLICATE KEY UPDATE permission = VALUES(permission), status = VALUES(status)`,
            [projectId, userId, permission, status]
        );
    ...
    /**
     * Get all access requests for a project (for owner)
     */
    ...
    async exists(projectId, userId) {
        const [rows] = await db.query(
            'SELECT id, status FROM project_access WHERE project_id = ? AND user_id = ? LIMIT 1',
            [projectId, userId]
        );
        return rows[0] || null;
    }
